...
## 2026-06-15 2.2.2 Hybrid GPU Low-Power And Home UI

- [x] Version scope: treat this as FanControl 2.2.2, focused on hybrid GPU protection, GPU unread display, and homepage readability.
- [x] TempBridge starts CPU-only by default and only enables GPU polling when low-power protection is off or the discrete GPU appears awake.
- [x] Add `gpuReadState` so GPU unread/not-polled state travels from TempBridge to Go core, Wails models, frontend UI, history, and tray status.
- [x] Default `gpuLowPowerProtection` to enabled for old and new configs; keep a settings toggle as an escape hatch for users who need forced dGPU reads.
- [x] Add a local `config/hardware-profile.json` cache for notebook GPU identification; missing or stale profiles are refreshed automatically, while sensor reconciliation is limited to startup/first valid sensor data to avoid runtime profile churn.
- [x] NVIDIA/NVML must not be used for default dGPU-awake probing because it can wake the dGPU; GPU temperature and power continue to use the existing TempBridge/LHM sensor core only after GPU reading is explicitly allowed.
- [x] Default GPU read mode is `auto`: TempBridge uses Windows DXGI + GPU performance counters as a light activity gate, then only enables LHM GPU polling while the dGPU appears active.
- [x] NVIDIA activity can appear as a dGPU process with 0% instantaneous counter utilization, but process-name fallback caused false positives such as PCManager; default auto mode now ignores process-only signals and waits for real engine utilization or dedicated-memory activity.
- [x] Auto mode now pauses GPU polling after roughly 30 seconds of idle dGPU activity and removes the heavier process fallback probe to reduce intermittent CPU spikes.
- [x] IPC response writes to already-closed GUI clients are downgraded to debug logs, so transient disconnects no longer appear as scary bridge failures.
- [x] THRM metric-card gauge styling is scoped to `.semi-gauge-svg`, preventing card icon SVG paths from being recolored into near-invisible strokes.
- [x] The settings UI exposes only `自动（推荐）` and `始终读取`; do not add a separate low-power-priority option unless user testing shows it is needed.
- [x] The GPU activity gate is split into `GpuActivityDetector.cs` and `GpuReadCoordinator.cs`; avoid adding more GPU probing code into the already-large `Program.cs`.
- [x] Portable packaging excludes `config.json` and `hardware-profile.json`; defaults are generated by the app, while installed-user configs are restored by the installer on upgrade.
- [x] Installer upgrades also back up and restore `config/hardware-profile.json`, so device/hardware profile information follows the user's existing install instead of being replaced by package defaults.
- [x] GPU not-polled data must not participate in max temperature, temperature rise prediction power samples, learning power samples, or history curves.
- [x] UI rule: GPU temperature not read displays as `未读取` or `--`, never as `0°C`; GPU power displays `0 W` when the dGPU is intentionally skipped.
- [x] Homepage visual pass: keep existing FanControl/THRM-style cards and theme variables, reduce density slightly, and avoid hardcoded theme-specific colors.
- [x] Release note draft added at `docs/release-notes/fancontrol-2.2.2.md`; do not publish until the user confirms.

## 2026-06-15 2.2.2 FlyDigi Visible Device Library And Disconnect Display

- [x] Supersedes the earlier hidden-FlyDigi beta decision: FlyDigi BS1/BS2/BS2PRO/BS3/BS3PRO are now normal visible built-in device profiles in the frontend device library.
- [x] `Legacy RPM controller` is retained only as an internal compatibility/control template; it must not be injected into the visible built-in device library or remain as a normalized saved device.
- [x] Native FlyDigi auto-match now persists the concrete FlyDigi profile ID, so the homepage, settings, curves, tray, and device-scoped learning use the matched device name instead of `builtin.legacy.rpm`.
- [x] Connecting a scanned native FlyDigi device writes `activeDeviceProfileId` and `activeDeviceProfileIdsByTransport` to the concrete FlyDigi profile, while keeping the legacy RPM send path available inside the device manager.
