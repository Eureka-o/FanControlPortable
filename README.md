...
- `FanControl-2.3.1-amd64-installer.exe`：推荐大多数用户使用。安装后可以从开始菜单或桌面启动，升级到 2.3.1 时会保留已有配置。
- `FanControl-2.3.1-portable.zip`：免安装便携版。解压到一个固定文件夹后运行 `FanControl.exe`。
...
- 优化飞智 BS 系列设备的转速与挡位控制逻辑，减少手动挡位、智能曲线和设备实际状态不同步的情况。
- 改进自动/手动模式切换时的状态处理；设备未成功应用时，不会继续显示成已切换成功。
- 修复部分兼容模式下曲线页可能显示百分比而不是实际转速的问题。
- 优化曲线页的趋势显示设置，隐藏部分信息后，首页缩略图也会保持同步；无 GPU 数据时不再显示异常的 0 度占位。
- 修复趋势曲线悬浮提示没有按展示设置顺序排列的问题。
- 修复小八 Plus 主题在曲线页部分卡片圆角和背景显示不一致的问题，并补全部分内置主题字体中缺失的主题名、设备名字符。
- 新增主题 DIY 说明文档，并提供基础版和高级版主题模板，方便用户基于现有主题系统制作和分享自己的主题。
- 优化安装包更新内置主题的方式，主题改为按版本同步，减少升级时反复覆盖本地主题文件的情况。
