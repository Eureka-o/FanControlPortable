...
- `FanControl-2.2.2-amd64-installer.exe`：推荐大多数用户使用。安装后可以从开始菜单或桌面启动，升级 2.0/2.1 到 2.2.2 时会保留已有配置。
- `FanControl-2.2.2-portable.zip`：免安装便携版。解压到一个固定文件夹后运行 `FanControl.exe`。
...
- 新增独显低功耗保护，混合模式下默认不唤醒未工作的独显。
- 新增本机显卡档案，用于识别 NVIDIA、Intel、AMD 显卡环境；档案缺失或与实际传感器不一致时会自动刷新。
- GPU 未读取时，温度显示为“未读取”或 `--`，功耗显示为 `0 W`。
- GPU 未读取的数据不会参与最高温、温升预判、学习机制和历史曲线。
- 飞智（FlyDigi）BS1、BS2、BS2PRO、BS3、BS3PRO 已加入内置设备库，连接后会显示对应设备名称。
- 优化短暂断联时的首页显示，减少瞬时掉线导致页面闪到“设备未连接”的情况。
- 优化首页信息层级，减少温度、风扇速度和功耗信息的拥挤感。
- 本版本不改变 WiFi 发包格式，不删除已有设备档案。
