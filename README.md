...
- 设置页新增统一的设备连接区域，扫描、发现、连接和当前设备状态集中展示。
- 首页连接按钮与设置页扫描逻辑保持一致，避免 WiFi 兼容设备被已连接的蓝牙/HID 设备抢占。
- WiFi 兼容模式支持浅扫描、深度扫描、动态 IP 兼容和手动添加地址；扫描结果会逐步显示，不再需要等待全部流程结束。

### 界面与主题

- 默认浅色/深色外观新增可切换的 WindowGlass 玻璃质感，侧边栏、顶部栏和主要卡片会使用半透明毛玻璃层级。
...
> 沙丘、小八 Plus、蜡笔小新等主题包含更多图片、半透明层和装饰元素；默认玻璃外观也会使用更多透明与模糊效果。如果在核显或低功耗模式下感觉界面不够流畅，可以关闭玻璃材质或切换到更轻量的主题。
