...
- `FanControl-2.3.0-amd64-installer.exe`：推荐大多数用户使用。安装后可以从开始菜单或桌面启动，升级 2.0/2.1/2.2 到 2.3.0 时会保留已有配置。
- `FanControl-2.3.0-portable.zip`：免安装便携版。解压到一个固定文件夹后运行 `FanControl.exe`。
...
- 新增 2 套内置主题：小八 Plus、蜡笔小新；它们使用更多图片、半透明层和装饰元素，低功耗设备可按需要切换到默认主题或较轻量主题。
- 自定义主题现在支持随主题文件夹携带图片、字体等相对资源，安装版和便携版都会优先从软件目录的 `themes` 文件夹加载。
- 优化首页、曲线页、设置页的卡片层级、圆角、开关和下拉交互，让自带主题与自制主题的显示更一致。
- 修复开机自启动状态显示可能与 Windows 实际自启动状态不同步的问题。
- 继续保留 2.2.x 的 BLE/HID 自动查找、FlyDigi 设备适配、WiFi 动态 IP 兼容、按设备隔离曲线和既有主题迁移能力。
