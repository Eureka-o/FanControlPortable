...
- `FanControl-2.4.2-amd64-installer.exe`：推荐大多数用户使用。安装后可以从开始菜单或桌面启动，升级到 2.4.2 时会保留已有配置。
- `FanControl-2.4.2-portable.zip`：免安装便携版。解压到一个固定文件夹后运行 `FanControl.exe`。
...
- 修复 Intel Core Ultra 9 275HX 等新平台可能缺少 CPU 温度与功耗的问题，并改善 PawnIO 临时读取失败后的恢复。
- 修复开机自启、设备晚到场和休眠唤醒后的自动连接；原生 HID/BLE 命中后会直接连接，不再等待 WiFi 浅扫描。
- 改善 BS1 蓝牙扫描、连接、GATT 探测和健康检查仲裁，失效连接可在后续读取时重新建立。
- 隔离旧重连任务与旧 IPC 连接，手动断开不会被旧任务反向连回，旧请求和事件也不会覆盖新连接状态。
- 智能温控会隔离缺失功耗、缓存遥测和异常温度；高负载温升时升速更跟手，静音学习更稳。
- 减少重复配置保存、前端回写、调试帧分配、日志写入和 WiFi 深扫预分配，降低后台无效工作量。
- 移除未使用的前端组件与依赖，并保持浅色/深色主题字体及生产包字体资源与参考软件一致。
