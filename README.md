...
- `FanControl-2.2.1-amd64-installer.exe`：推荐大多数用户使用。安装后可以从开始菜单或桌面启动，升级 2.0/2.1 到 2.2.1 时会保留已有配置。
- `FanControl-2.2.1-portable.zip`：免安装便携版。解压到一个固定文件夹后运行 `FanControl.exe`。
...
- 修复启动后设备显示可能回到默认设备的问题，首页会优先沿用上次启用的设备档案。
- 修复托盘设备名和速度单位显示不一致的问题，降低 Windows 悬停提示截断 `RPM` 的概率。
- 优化关于页面和右侧菜单显示，修复联系方式复制后的提示文案异常。
