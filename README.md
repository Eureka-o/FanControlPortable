...
| 设备 | 连接方式 | 控制单位 | 当前支持 |
| --- | --- | --- | --- |
| Slim压风散热器Pro | WiFi | 百分比 | 速度读取、速度下发、曲线控制、查找 IP、动态 IP 兼容、WiFi 智能启停 Beta |
...
- `FanControl-2.4.1-amd64-installer.exe`：推荐大多数用户使用。安装后可以从开始菜单或桌面启动，升级到 2.4.1 时会保留已有配置。
- `FanControl-2.4.1-portable.zip`：免安装便携版。解压到一个固定文件夹后运行 `FanControl.exe`。
...
- 优化设备连接体验：设置页扫描、首页连接、当前设备和候选设备展示更加一致，蓝牙、HID 与 WiFi 兼容设备的发现和连接更清晰。
- 优化 WiFi 兼容模式：保留手动添加地址、动态 IP 兼容和深度扫描能力，扫描设备时会按当前兼容模式自动处理。
- 优化后台运行流畅度：减少后台空闲时不必要的刷新、重复状态更新和轮询，长时间挂后台时资源占用波动更低。
- 优化主题资源加载：高级主题的图片与字体资源改为按需加载，减少大主题带来的启动压力和内存压力。
