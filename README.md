...
飞智设备会优先按内置档案自动匹配。设置页点击「连接原生设备」后会先遍历 HID 原生档案，再遍历 BLE 原生档案；用户保存的 BLE/HID 原生档案也会进入这个自动遍历过程。BLE/HID 不需要在高级设备页选择或启用；WiFi 和虚拟串口/COM 仍作为兼容设备手动选择。
...
- BLE/HID 原生设备改为点击连接后自动查找，用户 BLE/HID 档案可保留在高级设备页维护，但不再需要手动选择或启用。
...
- WiFi 和虚拟串口/COM 的通用档案能力已经具备基础运行时，不同 DIY 设备仍需要按主控协议填写正确字段；BLE/HID 原生设备请使用设置页的「连接原生设备」自动遍历。
