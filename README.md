...
- `FanControl-2.2.4-amd64-installer.exe`：推荐大多数用户使用。安装后可以从开始菜单或桌面启动，升级 2.0/2.1/2.2 到 2.2.4 时会保留已有配置。
- `FanControl-2.2.4-portable.zip`：免安装便携版。解压到一个固定文件夹后运行 `FanControl.exe`。
...
- BLE/HID 设备改为点击连接后自动查找，用户 BLE/HID 档案可保留在高级设备页维护，但不再需要手动选择或启用。
- FlyDigi BLE/HID 连接后会按实际 RPM 设备显示与控制，曲线方案、学习数据和手动速度会跟随当前连接设备隔离继承。
- WiFi 兼容设备连接时会做动态 IP 兼容式轻量查找；低功耗保护不轮询独显传感器时，首页仍会保留硬件档案中的 GPU 型号。
- 升级时会自动整理旧版本保存的 BS1 BLE / FlyDigi HID 连接状态，减少重启或健康检查后的反复连接失败。
- BS1 BLE 连接补充连接保持和兼容处理，提升连接后的稳定性。
- WiFi 和虚拟串口/COM 仍作为兼容设备手动选择，原有 IP、曲线、学习数据和用户设备会继续保留。
- 设置页和控制面板结构做了拆分整理，后续维护多连接方式时更稳。
