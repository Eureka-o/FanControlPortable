...
- `FanControl-2.2.5-amd64-installer.exe`：推荐大多数用户使用。安装后可以从开始菜单或桌面启动，升级 2.0/2.1/2.2 到 2.2.5 时会保留已有配置。
- `FanControl-2.2.5-portable.zip`：免安装便携版。解压到一个固定文件夹后运行 `FanControl.exe`。
...
- 新增 4 套内置主题：小八、Doro、耄耋、Cyberpunk 2077。前三套为浅色主题，Cyberpunk 2077 为深色主题。
- 新主题会随安装包和便携版一起提供，默认放在软件目录的 `themes` 文件夹，安装或解压后可直接在设置页选择。
- 修复 THRM 主题和部分自制主题左上角圆角区域可能出现黑块或露底的问题。
- 修复「温度与风扇历史详情」中 RPM 设备的风扇平均值单位显示错误。
- 继续保留 2.2.x 的 BLE/HID 自动查找、FlyDigi 设备适配、WiFi 动态 IP 兼容和按设备隔离曲线等能力。
