...
- `FanControl-2.5.2-amd64-installer.exe`：2.5.2 正式版安装包，升级时会保留已有配置。
- `FanControl-2.5.2-portable.zip`：2.5.2 正式版便携包，解压到固定文件夹后运行 `FanControl.exe`。
...
- 历史页将功耗与温度/风扇拆分为对齐的双图，同时减少切换曲线页时的重复请求和离屏渲染。
- 软件内更新支持校验、断点续传、暂停、继续、中止和失败重试，下载浮窗可收起、展开和拖动。
- 配置保存失败时保留原配置，并向界面返回明确失败结果。
- 精简重复中文字体资源，缩小安装包与便携包体积。
