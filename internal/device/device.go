...
func (m *Manager) SetFanSpeed(rpm int) bool {
	...
	if m.deviceType == types.DeviceTransportWiFi {
		...
		return m.setWiFiSpeedLocked(rpm)
	}
...
func (m *Manager) SetManualGear(gear, level string) bool {
	...
	if m.deviceType == types.DeviceTransportWiFi {
		...
		speed := types.FanSpeedMaxPercent
		if fanData != nil && fanData.TargetRPM > 0 {
			speed = int(fanData.TargetRPM)
		}
		return m.setWiFiSpeedLocked(speed)
	}
